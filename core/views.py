...
# ----------------------------
def _is_video_file(url_or_name):
    if not url_or_name:
        return False
    name = str(url_or_name).lower()
...
def home(request):
    ...
    if request.user.is_authenticated:
        user_posts_qs = Post.objects.filter(creator=request.user).order_by('-created_at')
        others_qs = Post.objects.exclude(id__in=watched).exclude(creator=request.user).order_by('-created_at')
        posts = list(user_posts_qs) + list(others_qs)
    else:
        posts = list(Post.objects.exclude(id__in=watched).order_by('-created_at'))

    for p in posts:
        p.is_video = _is_video_file(getattr(p, 'file', '') and getattr(p.file, 'url', getattr(p.file, 'name', '')))
